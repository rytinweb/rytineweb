      ...
      <main className="pt-header-h min-h-screen bg-white">
        ...
        <section className="bg-slate-50 border-b border-slate-200/80 text-gray-900 py-16 text-center relative overflow-hidden">
          <div className="relative z-10">
            ...
            <h1 className="text-3xl md:text-4xl lg:text-5xl font-black mb-4 text-gray-900">Recent Projects</h1>
        ...
        <section className="py-16 md:py-24 max-w-[1280px] mx-auto px-6 lg:px-8">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {projects.map((proj, idx) => (
              <div
                key={proj.title}
                className="group border border-slate-200 rounded-2xl overflow-hidden bg-white hover:bg-slate-50/40 shadow-sm hover:shadow-md transition-all duration-300 hover:-translate-y-1"
              >
                {/* Thumbnail Image placeholder */}
                <div className="aspect-[16/10] bg-slate-50 border-b border-slate-200/60 relative flex items-center justify-center p-6 overflow-hidden">
                  <div className="absolute inset-0 bg-[radial-gradient(circle,rgba(0,0,0,0.02)_1px,transparent_1px)] bg-[size:24px_24px]" />
                  <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-48 h-48 rounded-full bg-primary/5 filter blur-[40px] group-hover:bg-primary/10 transition-colors" />
                  
                  <div className="relative z-10 flex flex-col items-center gap-3 text-slate-400 group-hover:text-primary transition-colors">
                ...
                <div className="p-6">
                  ...
                  <div className="flex flex-wrap gap-1.5 font-light">
                    {proj.tags.map((tag) => (
                      <span key={tag} className="px-2.5 py-1 bg-slate-100 rounded-md text-[10px] text-slate-600 font-semibold">
                        {tag}
                      </span>
                    ))}
                  </div>
